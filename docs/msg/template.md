...
```
PUT ${BASE_URL}/msg/template/{tempId}
```
...
| `name`    | 字符串   | 否    | 名称                                       |
| `type`    | 字符串   | 否    | 类型（目前有两个有效值：msg_sms、msg_ussd）不填默认为 msg_sms |
| `content` | 字符串   | 否    | 模板内容  注意：实际发送内容，中文长度不超过62个字（包含标点符号）<br /> 1.无参数模板示例：<br /> 天猫双11全球狂欢节，全球好货不只5折。亿万优惠券10点准时开抢，现在打开淘宝或天猫客户端，开始抢红包啦！<br />2.带参数模板示例：<br />【支付宝】你的支付宝发生#*#元的交易，校验码：#*#打死都不能告诉别人哦！唯一热线95188 <br />1、变量用#*#代替 <br /> 2、内容+变量不能超过62字符 |
| `remark`  | 字符串   | 否    | 备注                                       |
