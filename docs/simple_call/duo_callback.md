...
用户应用调用该接口后， [yunhuni.com](http://yunhuni.com/) 首先向第一个被叫方发起呼叫；在第一方接听后，向二方放发起呼叫；第二方接听后，双方通话。期间任何一方挂机，就结束双向回拨过程。
...
| `ring_tone_mode`      | 枚举          | 选填，默认`3`            | 自定义回铃音播放模式。参见`参数详解`的`自定义回铃音播放模式列表`。      |
...
| `record_mode`**尚未开放** | 枚举          | 选填，默认`0`            | 录音模式。参见`参数详解`的`录音模式列表`。                  |
